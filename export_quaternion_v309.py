...
"""
---------------------------------------------------------------------------------------------------------------
DQ Blend Weights Tool for Autodesk Maya v3.0.9
==============================================
...
Инструмент DQ Blend Weights v3.0.9
==================================
...
PLUGIN_PATH = r"D:\Josep\source\repos\applyDQVertexColors\x64\Release\applyDQVertexColors.mll"
...
EXPORT_DIR = r"D:\export_dq_blend_weights"  # export folder
DECIMAL_PLACES = 4  # number of decimal places
SAVE_ZERO_WEIGHTS = True    # save zero weights
MERGE_COLORSET = True   # combine colorsets
CHECK_DQ_WEIGHTS = False    # check weights
...
# --- UI ---
def dq_weights_v309_ui():
    global MERGE_COLORSET, CHECK_DQ_WEIGHTS
    window_name = "dqWeightToolUI_v3.0.9"
    
    # --- IMPORTANT: Remove window preferences so width is not locked ---
    ...
    # Main Window (sizeable=True is key here)
    window = cmds.window(window_name, title="DQ Blend Weights Tool v3.0.9", widthHeight=(360, 500), sizeable=True)
    
    # We use a main column layout that adjusts to window width
    ...
    # =================================================================================
    # --- UI SECTION 1: MAIN BUTTONS (Always Visible) ---
    # =================================================================================
    cmds.separator(height=5)
    
    # 1. BATCH
    cmds.button(label="Export+Apply from Meshes (Batch)", height=40, bgc=(0.8, 0.6, 0.9), command=run_export_apply_meshes_batch)
    ...
    # =================================================================================
    # --- UI SECTION 2: COLLAPSIBLE SETTINGS & TOOLS ---
    # =================================================================================
    
    # Frame Layout: collapse=True makes it closed by default
    cmds.frameLayout(label="Settings & Manual Tools", collapsable=True, collapse=True, borderStyle="etchedIn", marginWidth=5, marginHeight=5)
    
    # Column Layout inside the Frame
    ...
    cmds.checkBox(label="Save Zeros", value=SAVE_ZERO_WEIGHTS, changeCommand=toggle_save_zero)
    cmds.checkBox(label="Merge Set", value=MERGE_COLORSET, changeCommand=toggle_merge_colorset)
    cmds.checkBox(label="Check DQ", value=CHECK_DQ_WEIGHTS, changeCommand=toggle_check_dq)
    cmds.setParent('..') # Back to frame column

    cmds.separator(height=5)
    
    # 3. Manual Export/Apply Buttons
    cmds.button(label="Export DQ from Mesh (Only)", height=25, command=run_export_mesh)
    cmds.button(label="Export DQ from Selection (Only)", height=25, command=run_export_verts)
    cmds.button(label="Apply Current JSON as Color", height=25, command=run_apply)

    cmds.setParent('..') # Close Frame Column
    cmds.setParent('..') # Close Frame Layout
    cmds.setParent('..') # Close Main Window Layout

    cmds.showWindow(window)


def export_quaternion_v309():
    """
    Function that launches the interface from the script manager
    """
    dq_weights_v309_ui()


if __name__ == '__main__':
    export_quaternion_v309()