...
class LocatorsTool2(object):
    ...
    def prepare_environment(self):
        # Тихая загрузка плагина, чтобы не спамить warning
...
# UI BUILDER
    def show_ui(self):
        ...
        self.window = cmds.window(WINDOW_NAME, title="Face Rig Pipeline 2019", widthHeight=(360, 850), sizeable=True)
        main_layout = cmds.columnLayout(adj=True, rowSpacing=5)

        # BLOCK 1 (Fixed: Removed borderStyle flag)
        cmds.frameLayout(label="1. Blendshapes & Locators Setup", marginWidth=5, marginHeight=5)
        ...
        # BLOCK 2 (Fixed: Removed borderStyle flag)
        cmds.frameLayout(label="2. Final Skin & Face BS Generator", marginWidth=5, marginHeight=5)
        cmds.columnLayout(adj=True, rowSpacing=5)
        ...
        # BLOCK 3 (Fixed: Removed borderStyle flag)
        cmds.frameLayout(label="3. Clean Skin (Max 4 Inf)", marginWidth=5, marginHeight=5)
        cmds.columnLayout(adj=True, rowSpacing=5)
...
# Обратите внимание: Мы не вызываем LocatorsTool() здесь!
# Это нужно для правильной работы import/reload через launcher.